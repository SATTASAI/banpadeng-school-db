...
import {
  ...
  handleUpdateAssigneeStatus,
} from "./routes/tasks.js";
import { handleListActiveUsers } from "./routes/users.js";
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (pathname === "/api/users" && method === "GET") {
        ...
        return await handleListActiveUsers(request, env, user);
      }

      if (pathname === "/api/tasks") {
        const user = await getCurrentUser(request, env);
        if (!user || !user.role) return jsonResponse({ error: "กรุณาเข้าสู่ระบบ" }, 401);
        if (method === "GET") return await handleListTasks(request, env, user);
        if (method === "POST") return await handleCreateTask(request, env, user);
      }
      ...
      if (taskMatch) {
        ...
        if (method === "GET") return await handleGetTask(request, env, user, taskId);
        if (method === "PATCH") return await handleUpdateTask(request, env, user, taskId);
        if (method === "DELETE") return await handleDeleteTask(request, env, user, taskId);
      }

      const taskAssigneesMatch = pathname.match(/^\/api\/tasks\/(\d+)\/assignees$/);
      if (taskAssigneesMatch && method === "POST") {
        const user = await getCurrentUser(request, env);
        if (!user || !user.role) return jsonResponse({ error: "กรุณาเข้าสู่ระบบ" }, 401);
        return await handleAddAssignees(request, env, user, Number(taskAssigneesMatch[1]));
      }

      const taskAssigneeMatch = pathname.match(/^\/api\/tasks\/(\d+)\/assignees\/(\d+)$/);
      if (taskAssigneeMatch) {
        const user = await getCurrentUser(request, env);
        if (!user || !user.role) return jsonResponse({ error: "กรุณาเข้าสู่ระบบ" }, 401);
        const taskId = Number(taskAssigneeMatch[1]);
        const targetUserId = Number(taskAssigneeMatch[2]);
        if (method === "PATCH") return await handleUpdateAssigneeStatus(request, env, user, taskId, targetUserId);
        if (method === "DELETE") return await handleRemoveAssignee(request, env, user, taskId, targetUserId);
      }
